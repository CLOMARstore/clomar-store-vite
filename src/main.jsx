...
const LABEL_LAYOUTS = {
  ...
  a4_vertical_3x3: { key: 'a4_vertical_3x3', label: 'A4 vertical · 3 columnas × 3 filas', paper: 'a4', columns: 3, rows: 3, width: 52, height: 80, gapX: 5, gapY: 5, density: 'vertical' },
...
const resolveAdaptiveLayout = (layoutKey = 'a4_3x7', orientation = 'auto', labelUse = 'auto') => {
  ...
  if (resolvedOrientation === 'vertical') return 'a4_vertical_3x3';
  if (layoutKey === 'a4_vertical_3x3') return 'a4_3x7';
  return layoutKey;
};
const resolveLabelComposition = (composition = 'auto', profile = 'general', orientation = 'horizontal', layout = {}) => {
  if (composition !== 'auto') {
    if (composition === 'two-column' && (orientation === 'vertical' || Number(layout.width || 0) < 58)) return 'classic';
    return composition;
  }
...
<style>
  ...
  *{box-sizing:border-box} html,body{margin:0;padding:0;background:#fff;color:#000;font-family:Arial,Helvetica,sans-serif} body{print-color-adjust:exact;-webkit-print-color-adjust:exact}
  ...
  .label.orientation-vertical{padding:2.6mm 2.6mm 2.3mm;grid-template-rows:4.5mm 15mm 10.5mm 38mm;gap:1.2mm;border-radius:3mm}.label.orientation-vertical .brand{min-height:4.5mm;font-size:8px}.label.orientation-vertical .brand img{width:4.2mm;height:4.2mm}.label.orientation-vertical .label-title{height:15mm}.label.orientation-vertical .name{height:11.4mm;max-height:11.4mm;font-size:12.1px;line-height:1.05}.label.orientation-vertical .name.name-medium{font-size:11px}.label.orientation-vertical .name.name-long{font-size:9.8px}.label.orientation-vertical .meta{height:2.2mm;font-size:7px}.label.orientation-vertical .price{min-height:10.5mm;border-radius:2mm;padding:1.25mm}.label.orientation-vertical .price span{font-size:6.3px}.label.orientation-vertical .price strong{font-size:21px}.label.orientation-vertical .codes{height:38mm;align-items:end}.label.orientation-vertical .barcode-wrap{width:100%;grid-template-rows:20mm auto}.label.orientation-vertical .barcode-svg{height:20mm}.label.orientation-vertical .code-text{font-size:7.2px;margin-top:.7mm}.label.orientation-vertical.mode-both .qr-wrap{grid-template-columns:16mm;grid-template-rows:16mm auto}.label.orientation-vertical.mode-both .qr{width:16mm;height:16mm}.label.orientation-vertical.mode-both .barcode-wrap{width:calc(var(--label-w) - 22mm);grid-template-rows:15mm auto}.label.orientation-vertical.mode-both .barcode-svg{height:15mm}.label.orientation-vertical.mode-qr .qr-wrap{grid-template-columns:29mm;grid-template-rows:29mm auto}.label.orientation-vertical.mode-qr .qr{width:29mm;height:29mm}.label.orientation-vertical.mode-qr .qr-wrap small{font-size:6.3px;font-weight:900}
...
function LabelsAdmin({ products = [], categories = [], subcategories = [], store }) {
  ...
  function printLabels() {
    ...
    openLabelsPrintWindow({ items: printableItems, store, mode, showPrice, showLogo, showCodeText, sheetLayout: effectiveSheetLayout, labelStyle: labelSize, labelTemplate, labelUse, orientation: activeOrientation, composition, catalogUrl: stableCatalogUrl });
  }
  ...
  return <div className="page labels-page labels-pro-page labels-v342-page">
    <div className="hero labels-pro-hero labels-v342-hero"><div><span className="eyebrow">V03.4.3 · Orientación, composición y perfil comercial</span><h1>Etiquetas adaptativas para cada tipo de producto</h1><p>Seleccione rubro, finalidad, orientación y composición. El sistema prioriza precio, QR o barcode sin saturar la etiqueta.</p></div><div className="labels-v342-hero-actions"><span className={stableCatalogUrl ? 'labels-url-state ready' : 'labels-url-state'}>{stableCatalogUrl ? 'QR público configurado' : 'QR público pendiente'}</span><button className="secondary-btn" onClick={testCatalogUrl}>Probar catálogo público</button></div></div>
